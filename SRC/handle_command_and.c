    ...
    char **env)
{
    ...
    while (and_cmds[i] != NULL) {
        if (such_or(and_cmds[i])) {
            a->commands[a->i] = and_cmds[i];
            handle_or(a, env);
...
static int ends_with_and(char *line)
{
...
int handle_and(principal_function_t *a, char **env)
{
    ...
    if (!dear_and(a->commands[a->i]))
        return 0;
    if (only_and(a->commands[a->i]))
        return 1;
    if (ends_with_and(a->commands[a->i])) {
        dprintf(2, "Invalid null command.\n");
        return 1;
    }
    and_cmds = parse_and(a->commands[a->i]);
    if (and_cmds == NULL || and_cmds[0] == NULL) {
        free(and_cmds);
        return 1;
    }
